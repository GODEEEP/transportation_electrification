...
#         v_days_df = pd.read_csv(os.path.join(os.getcwd(), 'data', 'fleet-schedules', f'{fleet}', 'veh_op_days.csv'))
#         v_scheds_df = pd.read_csv(os.path.join(os.getcwd(), 'data', 'fleet-schedules', f'{fleet}', 'veh_schedules.csv'))
        
        v_days_df  = pd.read_csv(os.path.join('..', 'output', f'{veh_type}_veh_op_days.csv'))
        v_scheds_df = pd.read_csv(os.path.join('..', 'output', f'{veh_type}_veh_schedules.csv'))
...
#         charge_profs_df_all = pd.DataFrame()
#        fleet_prof_all_samples = pd.DataFrame()
        
        for rand_int in rand_ints:
            v_days_sample_df = v_days_df.sample(fleet_size, 
                                                replace=True,
                                                random_state=rand_int)
            # display(v_days_sample_df)
            ...
            for i, vday in v_days_sample_df.iterrows():
                ...
                if charge_strat == 'min_power':
                    ...
                    total_energy_kwh = vday.kwh_per_mile * vday.vmt
                    ...
                    power_kw   = random.choices(kw_list, weights = kw_weights, k=1)
                    power_kw   = power_kw[0] # consistent sequence loops except we change n_samples and fllet_size because of random.seed
                    ...
                    while i < len(three_vday_scheds_df):
                        ...
                        if i == len(three_vday_scheds_df) - 1:
                            ...
                            if row.on_shift == next_row.on_shift:
                                ...
                                kwhs.append(row.kwh_per_mile+next_row.kwh_per_mile)
                                i+=2
                            else:
                                start_times.append(row.start_time)
                                end_times.append(row.end_time)
                    ...
                    for i, row in three_vday_scheds_df.iterrows():
                        if row.on_shift == 1: #if: on-shift...
                            net_energy_consumed_kwh += (row.vmt * row.kwh_per_mile) #add energy
            ...
            # charge_profs_df_all  = pd.concat([charge_profs_df_all, charge_profs_df]).reset_index(drop=True)
            ...
            #
            # print("charge_df:", len(charge_profs_df))
            fleet_prof_df = charge_profs_df.groupby('rel_s')['kw'].sum()
            fleet_prof_df = fleet_prof_df.reset_index()
            # print("fleet_df:",len(fleet_prof_df))

        #     fleet_prof_all_samples = pd.concat([fleet_prof_all_samples, fleet_prof_df]).reset_index(drop=True)
        #     print("all samples:", len(fleet_prof_all_samples))
        # print("\n Sample completed \n")
        #     fleet_prof_avg_df  = fleet_prof_all_samples.groupby('rel_s')['kw'].mean()
        #     fleet_prof_avg_df  = fleet_prof_avg_df.reset_index()
        # print("avg:", len(fleet_prof_avg_df))
            ...
            # print("all samples:", len(total_load_all_samples))
...
#         # Plot average fleet daily load profile
        ...
        # print(len(avg_load_all_samples))
...
#         if to_file==True:
#             # Save plot as .png
#             plot_fp = os.path.join(os.getcwd(), 'figures', f'{fleet_size}vehs_{charge_strat}.png')
...
#             avg_lp_fp = os.path.join(os.getcwd(), 'data', 'outputs', f'{fleet_size}vehs_avg-prof_{charge_strat}_{res}.csv')
...
#             pk_lp_fp = os.path.join(os.getcwd(), 'data', 'outputs', f'{fleet_size}vehs_peak-prof_{charge_strat}_{res}.csv') 
...
#             min_lp_fp = os.path.join(os.getcwd(), 'data', 'outputs', f'{fleet_size}vehs_min-prof_{charge_strat}_{res}.csv')
